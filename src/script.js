import {cardList,trunc,deleteProduct,usersBtn,deleteUser} from "./request.js";
        ...
        const userHtml = `
            <div class="user-card">
                ...
                <div>
                    ...
                    <button type="button" data-user="${user.id}"class="btn btn-danger delete-user-btns">delete</button>
                </div>
            </div>
        `;
    ...
    axios.get("https://api.escuelajs.co/api/v1/users")
        .then(response => {
          renderUsers(response.data)
            ...
            const deleteBtns = document.querySelectorAll(".delete-user-btns");
            deleteBtns.forEach((btn) => {
                btn.addEventListener("click", () => {
...
const addUser = () => {
    ...
    const email = document.querySelector(".email-deskr").value;
    ...
    if (email.includes("@gmail.com")) {
        const userToAdd = {
            ...
            password,
            role,
            avatar:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYxoulT_o1sU7vh1WqbSB1JE9P3AAYbtzylEDSLWRdVQ&s"
        };

        axios.post("https://api.escuelajs.co/api/v1/users", userToAdd)
    } else {
        alert("Будь ласка, введіть дійсну адресу Gmail");
    }
};
